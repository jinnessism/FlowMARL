...
class TrainState(flax.struct.PyTreeNode):
    ...
    def save(self, path):
        ...
        flat_params_str = {'/'.join(k): np.asarray(v) for k, v in flat_params.items()}
        np.savez(path, **flat_params_str)

    def load(self, path):
        import numpy as np
        from flax import traverse_util
        with np.load(path, allow_pickle=True) as data:
            flat_params_str = {tuple(k.split('/')): jnp.asarray(v) for k, v in data.items()}
        unflat_params = traverse_util.unflatten_dict(flat_params_str)
        def recursive_update(target, source):
            t_dict = target._dict if hasattr(target, '_dict') else target
            s_dict = source._dict if hasattr(source, '_dict') else source
            for k, v in s_dict.items():
                if k in t_dict:
                    t_val = t_dict[k]
                    if isinstance(t_val, dict) or hasattr(t_val, '_dict'):
                        recursive_update(t_val, v)
                    else:
                        t_dict[k] = v
                else:
                    t_dict[k] = v
        recursive_update(self.params, unflat_params)
